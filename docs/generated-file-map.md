# CoStar Generated File Map

This file is generated by `npm run docs:file-map`.
The generated file itself is excluded from the file count: `docs/generated-file-map.md`.
Branch and commit metadata are intentionally omitted so the file stays stable after commits and merges.

## Repository Snapshot

- Tracked / public candidate files counted: 267

## Top-Level Map

| Area | File count | Purpose |
| --- | ---: | --- |
| `.github` | 7 | GitHub community, issue, PR, and CI metadata |
| `(root)` | 17 | Repository metadata and entry docs |
| `assets` | 3 | Branding and public assets |
| `bin` | 1 | CLI entrypoint |
| `costar-core` | 85 | Shared core, stores, host-model tools, commits, MCP bridge |
| `docs` | 16 | Public architecture, support, testing, and product notes |
| `examples` | 13 | Public mock examples |
| `integrations` | 43 | Host adapters for Claude, Codex, and OpenClaw |
| `relationship-briefing` | 9 | Brief generation |
| `relationship-capture` | 10 | User-facing import orchestration |
| `relationship-graph` | 17 | Graph and review resolution |
| `relationship-ingestion` | 17 | Extraction and profile review |
| `relationship-profile` | 9 | Durable profile read / write |
| `relationship-roleplay` | 8 | Conversation simulation |
| `relationship-view` | 8 | Persistent markdown views |
| `scripts` | 4 | Checks, smoke tests, and generated docs |

## Important Entrypoints

- `bin/costar.mjs`: CLI for engine mode and host-model installs.
- `costar-core/`: shared stores, commits, host tool contracts, and MCP bridge.
- `integrations/claude/`: Claude host-model bundle.
- `integrations/codex/`: Codex host-model skill bundle.
- `integrations/openclaw/`: OpenClaw host-model bundle.
- `relationship-briefing/`: Briefing runtime and enhanced insight prompt.
- `scripts/check-public-repo.mjs`: public hygiene scanner.

## Full File List

### .github

- `.github/CODEOWNERS`
- `.github/FUNDING.yml`
- `.github/ISSUE_TEMPLATE/bug_report.yml`
- `.github/ISSUE_TEMPLATE/feature_request.yml`
- `.github/ISSUE_TEMPLATE/use_case.yml`
- `.github/PULL_REQUEST_TEMPLATE.md`
- `.github/workflows/ci.yml`

### (root)

- `.editorconfig`
- `.gitattributes`
- `.gitignore`
- `CHANGELOG.md`
- `CODE_OF_CONDUCT.md`
- `CONTRIBUTING.md`
- `eslint.config.mjs`
- `LICENSE`
- `NOTICE`
- `package-lock.json`
- `package.json`
- `README.md`
- `README.zh-CN.md`
- `RELEASE_NOTES_v0.1.0.md`
- `ROADMAP.md`
- `SECURITY.md`
- `START_HERE.md`

### assets

- `assets/.gitkeep`
- `assets/branding/costar.png`
- `assets/README.md`

### bin

- `bin/costar.mjs`

### costar-core

- `costar-core/artifacts/capture-artifacts.mjs`
- `costar-core/artifacts/README.md`
- `costar-core/artifacts/review-artifacts.mjs`
- `costar-core/commit/commit-log-store.mjs`
- `costar-core/commit/costar-commit.mjs`
- `costar-core/commit/README.md`
- `costar-core/host-install/host-installer-smoke.mjs`
- `costar-core/host-install/host-installer.mjs`
- `costar-core/host-model-adapter/catpaw-contract-smoke.mjs`
- `costar-core/host-model-adapter/claude-test-pack-smoke.mjs`
- `costar-core/host-model-adapter/codex-test-pack-smoke.mjs`
- `costar-core/host-model-adapter/conversation-spec-smoke.mjs`
- `costar-core/host-model-adapter/conversation-spec.mjs`
- `costar-core/host-model-adapter/extraction-guardrails-smoke.mjs`
- `costar-core/host-model-adapter/host-adapter-smoke.mjs`
- `costar-core/host-model-adapter/host-cli-smoke.mjs`
- `costar-core/host-model-adapter/host-transcript-smoke.mjs`
- `costar-core/host-model-adapter/openclaw-bootstrap-smoke.mjs`
- `costar-core/host-model-adapter/openclaw-test-pack-smoke.mjs`
- `costar-core/host-model-adapter/README.md`
- `costar-core/host-model-adapter/render-host-prompt-packet.mjs`
- `costar-core/host-model-adapter/render-host-session-protocol.mjs`
- `costar-core/host-model-adapter/review-protocol-smoke.mjs`
- `costar-core/host-model-adapter/review-protocol.mjs`
- `costar-core/host-model-adapter/run-host-tool.mjs`
- `costar-core/host-model-adapter/samples/briefing-generate.request.example.json`
- `costar-core/host-model-adapter/samples/capture-ingest.request.example.json`
- `costar-core/host-model-adapter/samples/commit-decisions.request.example.json`
- `costar-core/host-model-adapter/samples/profile-get.request.example.json`
- `costar-core/host-model-adapter/samples/review-prepare.graph.request.example.json`
- `costar-core/host-model-adapter/samples/review-prepare.profile.request.example.json`
- `costar-core/host-model-adapter/samples/review-protocol.graph-answer.example.json`
- `costar-core/host-model-adapter/samples/review-protocol.graph-input.example.json`
- `costar-core/host-model-adapter/samples/review-protocol.profile-answer.example.json`
- `costar-core/host-model-adapter/samples/review-protocol.profile-input.example.json`
- `costar-core/host-model-adapter/samples/review-translate.graph.request.example.json`
- `costar-core/host-model-adapter/samples/review-translate.profile.request.example.json`
- `costar-core/host-model-adapter/samples/roleplay-generate.request.example.json`
- `costar-core/host-model-e2e/host-model-e2e-checklist.md`
- `costar-core/host-model-e2e/README.md`
- `costar-core/host-model-e2e/runtime/codex-clean-insight-regression-smoke.mjs`
- `costar-core/host-model-e2e/runtime/host-model-e2e-smoke.mjs`
- `costar-core/host-model-e2e/runtime/host-model-persistence-graph-smoke.mjs`
- `costar-core/host-model-e2e/scenarios/mock-cold-start.json`
- `costar-core/host-model-e2e/scenarios/mock-incremental-update.json`
- `costar-core/host-model-workflows/briefing-workflow.mjs`
- `costar-core/host-model-workflows/capture-workflow.mjs`
- `costar-core/host-model-workflows/extraction-guardrails.mjs`
- `costar-core/host-model-workflows/README.md`
- `costar-core/host-model-workflows/roleplay-workflow.mjs`
- `costar-core/mcp/claude-bootstrap-smoke.mjs`
- `costar-core/mcp/claude-bundle-doctor-smoke.mjs`
- `costar-core/mcp/claude-clean-install-smoke.mjs`
- `costar-core/mcp/claude-config-install-smoke.mjs`
- `costar-core/mcp/claude-final-user-pack-smoke.mjs`
- `costar-core/mcp/costar-mcp-server.mjs`
- `costar-core/mcp/mcp-smoke.mjs`
- `costar-core/memory/embedding-utils.mjs`
- `costar-core/memory/memory-briefing-smoke.mjs`
- `costar-core/memory/memory-candidates-smoke.mjs`
- `costar-core/memory/memory-candidates.mjs`
- `costar-core/memory/memory-commit-smoke.mjs`
- `costar-core/memory/memory-commit.mjs`
- `costar-core/memory/memory-feedback-smoke.mjs`
- `costar-core/memory/memory-feedback.mjs`
- `costar-core/memory/memory-ids.mjs`
- `costar-core/memory/memory-lint-smoke.mjs`
- `costar-core/memory/memory-lint.mjs`
- `costar-core/memory/memory-retrieval.mjs`
- `costar-core/memory/memory-review.mjs`
- `costar-core/memory/memory-store-smoke.mjs`
- `costar-core/memory/memory-store.mjs`
- `costar-core/memory/samples/memory-candidates.request.example.json`
- `costar-core/memory/samples/memory-store.empty.example.json`
- `costar-core/memory/schemas/memory-store.schema.json`
- `costar-core/package.json`
- `costar-core/relationship-insights.mjs`
- `costar-core/stores/graph-review-store.mjs`
- `costar-core/stores/json-store-utils.mjs`
- `costar-core/stores/profile-store.mjs`
- `costar-core/stores/README.md`
- `costar-core/stores/view-store.mjs`
- `costar-core/tools/host-model-dispatcher.mjs`
- `costar-core/tools/README.md`
- `costar-core/tools/tool-contract.mjs`

### docs

- `docs/architecture.md`
- `docs/codex-acceptance-test-manual.md`
- `docs/comparison.md`
- `docs/host-adapter-public-hygiene.md`
- `docs/memory-v0.3.md`
- `docs/pitch-en.md`
- `docs/pitch-zh.md`
- `docs/product-shell-handoff.md`
- `docs/README.md`
- `docs/superpowers/plans/2026-04-30-v03-memory-mvp-acceptance.md`
- `docs/superpowers/plans/2026-04-30-v03-memory-mvp-implementation.md`
- `docs/superpowers/plans/2026-04-30-v031-memory-feedback-loop.md`
- `docs/superpowers/plans/2026-05-06-v032-scope-correction.md`
- `docs/superpowers/specs/2026-05-06-v032-scope-correction-design.md`
- `docs/support-matrix.md`
- `docs/tester-package.md`

### examples

- `examples/01-capture-to-briefing/briefing-input.json`
- `examples/01-capture-to-briefing/expected-output.md`
- `examples/01-capture-to-briefing/input.json`
- `examples/01-capture-to-briefing/README.md`
- `examples/02-profile-refresh/expected-output.md`
- `examples/02-profile-refresh/input.json`
- `examples/02-profile-refresh/profile-input.json`
- `examples/02-profile-refresh/README.md`
- `examples/03-graph-review/expected-output.md`
- `examples/03-graph-review/graph-review-input.json`
- `examples/03-graph-review/input.json`
- `examples/03-graph-review/README.md`
- `examples/README.md`

### integrations

- `integrations/claude/bootstrap-claude.ps1`
- `integrations/claude/claude-code.mcp.json`
- `integrations/claude/claude-desktop.mcp.json`
- `integrations/claude/doctor-claude-install.mjs`
- `integrations/claude/FINAL_USER_ACCEPTANCE.md`
- `integrations/claude/FINAL_USER_RESULTS_TEMPLATE.md`
- `integrations/claude/FIRST_SESSION.md`
- `integrations/claude/install-claude-config.mjs`
- `integrations/claude/install-claude-config.ps1`
- `integrations/claude/manifest.json`
- `integrations/claude/MOCK_TRANSCRIPT.md`
- `integrations/claude/PROMPT_PACKET.md`
- `integrations/claude/QUICKSTART.md`
- `integrations/claude/README.md`
- `integrations/claude/run-costar-mcp.mjs`
- `integrations/claude/sample-workflow.md`
- `integrations/claude/SESSION_PROTOCOL.md`
- `integrations/claude/TEST_PACK.md`
- `integrations/claude/TEST_REQUIREMENTS.md`
- `integrations/claude/TEST_RESULTS_TEMPLATE.md`
- `integrations/claude/tool-exposure.json`
- `integrations/codex/bootstrap-codex.ps1`
- `integrations/codex/costar/SKILL.md`
- `integrations/codex/MOCK_TRANSCRIPT.md`
- `integrations/codex/PROMPT_PACKET.md`
- `integrations/codex/README.md`
- `integrations/codex/sample-workflow.md`
- `integrations/codex/SESSION_PROTOCOL.md`
- `integrations/codex/TEST_PACK.md`
- `integrations/codex/TEST_RESULTS_TEMPLATE.md`
- `integrations/codex/tool-exposure.json`
- `integrations/openclaw/bootstrap-costar.ps1`
- `integrations/openclaw/CoStar/README.md`
- `integrations/openclaw/CoStar/SKILL.md`
- `integrations/openclaw/LOCAL_CLAW_TEST_GUIDE.md`
- `integrations/openclaw/MOCK_TRANSCRIPT.md`
- `integrations/openclaw/PROMPT_PACKET.md`
- `integrations/openclaw/README.md`
- `integrations/openclaw/sample-workflow.md`
- `integrations/openclaw/SESSION_PROTOCOL.md`
- `integrations/openclaw/TEST_PACK.md`
- `integrations/openclaw/TEST_RESULTS_TEMPLATE.md`
- `integrations/openclaw/tool-exposure.json`

### relationship-briefing

- `relationship-briefing/briefings/.gitkeep`
- `relationship-briefing/prompts/relationship-briefing.system.prompt.md`
- `relationship-briefing/README.md`
- `relationship-briefing/runtime/relationship-briefing.mjs`
- `relationship-briefing/runtime/run-relationship-briefing.mjs`
- `relationship-briefing/samples/relationship-briefing.request.example.json`
- `relationship-briefing/samples/relationship-briefing.response.example.json`
- `relationship-briefing/schemas/relationship-briefing.input.schema.json`
- `relationship-briefing/schemas/relationship-briefing.output.schema.json`

### relationship-capture

- `relationship-capture/README.md`
- `relationship-capture/runtime/relationship-capture.mjs`
- `relationship-capture/runtime/render-relationship-capture-summary.mjs`
- `relationship-capture/runtime/run-relationship-capture.mjs`
- `relationship-capture/samples/relationship-capture.request.commit.example.json`
- `relationship-capture/samples/relationship-capture.request.ingest.example.json`
- `relationship-capture/samples/relationship-capture.response.commit.example.json`
- `relationship-capture/samples/relationship-capture.response.ingest.example.json`
- `relationship-capture/schemas/relationship-capture.input.schema.json`
- `relationship-capture/schemas/relationship-capture.output.schema.json`

### relationship-graph

- `relationship-graph/README.md`
- `relationship-graph/runtime/relationship-graph-review-resolution.mjs`
- `relationship-graph/runtime/relationship-graph.mjs`
- `relationship-graph/runtime/run-relationship-graph-review-resolution.mjs`
- `relationship-graph/runtime/run-relationship-graph.mjs`
- `relationship-graph/samples/relationship-graph-review-resolution.request.example.json`
- `relationship-graph/samples/relationship-graph-review-resolution.response.example.json`
- `relationship-graph/samples/relationship-graph.request.find-path.example.json`
- `relationship-graph/samples/relationship-graph.request.get-person-graph.example.json`
- `relationship-graph/samples/relationship-graph.request.summarize-network.example.json`
- `relationship-graph/samples/relationship-graph.response.find-path.example.json`
- `relationship-graph/samples/relationship-graph.response.get-person-graph.example.json`
- `relationship-graph/samples/relationship-graph.response.summarize-network.example.json`
- `relationship-graph/schemas/relationship-graph-review-resolution.input.schema.json`
- `relationship-graph/schemas/relationship-graph-review-resolution.output.schema.json`
- `relationship-graph/schemas/relationship-graph.input.schema.json`
- `relationship-graph/schemas/relationship-graph.output.schema.json`

### relationship-ingestion

- `relationship-ingestion/prompts/relationship-ingestion.system.prompt.md`
- `relationship-ingestion/README.md`
- `relationship-ingestion/runtime/model-config.template.json`
- `relationship-ingestion/runtime/relationship-ingestion.mjs`
- `relationship-ingestion/runtime/relationship-review-resolution.mjs`
- `relationship-ingestion/runtime/run-relationship-ingestion.mjs`
- `relationship-ingestion/runtime/run-relationship-review-resolution.mjs`
- `relationship-ingestion/samples/relationship-ingestion.request.example.json`
- `relationship-ingestion/samples/relationship-ingestion.response.example.json`
- `relationship-ingestion/samples/relationship-review-resolution.request.example.json`
- `relationship-ingestion/samples/relationship-review-resolution.response.example.json`
- `relationship-ingestion/schemas/model-config.schema.json`
- `relationship-ingestion/schemas/relationship-ingestion.input.schema.json`
- `relationship-ingestion/schemas/relationship-ingestion.output.schema.json`
- `relationship-ingestion/schemas/relationship-profile.schema.json`
- `relationship-ingestion/schemas/relationship-review-resolution.input.schema.json`
- `relationship-ingestion/schemas/relationship-review-resolution.output.schema.json`

### relationship-profile

- `relationship-profile/README.md`
- `relationship-profile/runtime/relationship-profile.mjs`
- `relationship-profile/runtime/run-relationship-profile.mjs`
- `relationship-profile/samples/relationship-profile.request.get.example.json`
- `relationship-profile/samples/relationship-profile.request.patch.example.json`
- `relationship-profile/samples/relationship-profile.response.get.example.json`
- `relationship-profile/samples/relationship-profile.response.patch.example.json`
- `relationship-profile/schemas/relationship-profile.input.schema.json`
- `relationship-profile/schemas/relationship-profile.output.schema.json`

### relationship-roleplay

- `relationship-roleplay/prompts/relationship-roleplay.system.prompt.md`
- `relationship-roleplay/README.md`
- `relationship-roleplay/runtime/relationship-roleplay.mjs`
- `relationship-roleplay/runtime/run-relationship-roleplay.mjs`
- `relationship-roleplay/samples/relationship-roleplay.request.example.json`
- `relationship-roleplay/samples/relationship-roleplay.response.example.json`
- `relationship-roleplay/schemas/relationship-roleplay.input.schema.json`
- `relationship-roleplay/schemas/relationship-roleplay.output.schema.json`

### relationship-view

- `relationship-view/README.md`
- `relationship-view/runtime/relationship-view.mjs`
- `relationship-view/runtime/run-relationship-view.mjs`
- `relationship-view/samples/relationship-view.request.refresh-person.example.json`
- `relationship-view/samples/relationship-view.response.refresh-person.example.json`
- `relationship-view/schemas/relationship-view.input.schema.json`
- `relationship-view/schemas/relationship-view.output.schema.json`
- `relationship-view/views/.gitkeep`

### scripts

- `scripts/briefing-insights-smoke.mjs`
- `scripts/check-public-repo.mjs`
- `scripts/file-map-no-git-smoke.mjs`
- `scripts/generate-file-map.mjs`
