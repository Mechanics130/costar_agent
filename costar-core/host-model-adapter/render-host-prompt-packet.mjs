...
function renderPromptPacket(host) {
  ...
  lines.push(
    ...
    "",
    "## Canonical workflow 4: record feedback and learn safely",
    "",
    ...
    "3. If CoStar returns a reflection candidate, call `memory_reflection_prepare_cards` and ask the user to confirm why the prior extraction or briefing was wrong.",
    "4. Call `memory_reflection_commit` only after explicit user confirmation.",
    "5. Before later capture or briefing runs, call `memory_hints_get` to retrieve confirmed hints and include them in host reasoning.",
    "6. Use `memory_feedback_report` to inspect whether repeated corrections point to over-inference, stale facts, missing fields, or low-value briefing sections.",
    "",
    "## Canonical workflow 5: review graph edges",
    "",
