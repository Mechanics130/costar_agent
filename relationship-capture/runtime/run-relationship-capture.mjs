...
import { runRelationshipCapture } from "./relationship-capture.mjs";
...
if (!requestPath) {
  console.error("Usage: node run-relationship-capture.mjs <request-json-path> [output-json-path]");
  process.exit(1);
}
...
try {
  const result = await runRelationshipCapture(payload, fallbackConfig);
